...
const listCities = async (req, res) => {
  try {
    ...
    console.log("inside list cities");
    console.log(cityName);

    let data = await Axios.get(
      `http://api.weatherapi.com/v1/search.json?key=${process.env.WEATHER_KEY}&q=${cityName}`
    );
    ...
    return res
      ...
      .json({
        status: 200,
        data: data.data,
        message: "list city successfully",
      });
  } catch (error) {
    const requestedData = await WeatherData.findOne({
      where: { location: { name: "Ahmedabad" } },
    ...
    return res.render("wet", { res: requestedData, error: error });
  }
};
